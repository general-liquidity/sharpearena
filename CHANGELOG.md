...
### Known limitation
- The corrected standardized moments and the corrected inventory/cash arithmetic are fixed in SharpeBench, not here. This workspace pins `sharpebench-core` and `sharpebench-sim` at `=0.15.0`, which still carries the superseded estimators, so those repairs reach SharpeArena only through a SharpeBench release and a pin bump. Until then the corrected diagnostics stay under separate names and `deflated_sharpe_ci` is withheld rather than presented as agreeing with the pinned scorer. No committed artifact was rescored.

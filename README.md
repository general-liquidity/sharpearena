...
**What the evidence shows.** Ten committed experiments in [`paper/`](paper/) (every number from a script, JSON evidence, and fixed seeds) report, among their findings:
...
- **The concave ablation is an honest null.** At permanent-impact exponents **0.5 and 0.7**, the same pump remains unprofitable at every sampled point, but three of 22 confidence intervals cross zero and the tested sub-unit flow scale amplifies impact. An asymmetric large-flow positive control remains open.
- **Bounded public seed bands are invertible.** A causal AR adversary finds no undisclosed stressed-tier edge, but a scan of a public **2^16** seed band recovers **16/16** seeds from one observed bar in about one second. Unpredictability rests on seed entropy and secrecy, not transform hardness.

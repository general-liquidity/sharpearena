...
- **The realism gate fails the canonical tape, and says so.** The three calibrated stylized-facts checks fail the canonical configuration on **22 of 24** runs (0/8 Calm, 1/8 Hard, 1/8 Extreme). The finite-panel Fano statistic is reported separately as exploratory because its null is not calibrated. The opt-in `vol_clustering` knob is a partial, in-sample remediation: at strength 0.5 the clustering fact rises from 5/1/1 to **8/8/5** of 8 seeds per tier, and the three-check pass counts become 0/8, 8/8 and 5/8. At 0.0, the leaderboard configuration, the tape is byte-identical, so certification is reported beside the results rather than required by them.

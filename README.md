...
| **Manipulation red-team probe** | A diagnostic aimed at the simulator, not a strategy (the module ships an explicit disclaimer): a crude pump-and-dump schedule against the Kyle + Almgren-Chriss impact model, an `impact_boundary_sweep` locating where along the impact axis the pump stops paying, a `size_response` check that profit turns over rather than growing with size, and paired zero-impact reference runs (`kyle_lambda = eta = 0`, same seed and schedule) so `impact_pnl` attributes profit to having moved the price. Its trajectories double as positive examples for process gates. |

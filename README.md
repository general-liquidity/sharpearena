...
- **The realism gate fails the canonical tape, and says so.** The stylized-facts certification fails the canonical configuration on **23 of 24** runs (0/8 Calm, 0/8 Hard, 1/8 Extreme); volatility clustering and the Fano factor bind on the stressed tiers. The opt-in `vol_clustering` knob is the partial remediation: at strength 0.5 the clustering fact rises from 5/1/1 to **8/8/5** of 8 seeds per tier. At 0.0, the leaderboard configuration, the tape is byte-identical, so certification is reported beside the results rather than required by them.
...
- **The concave ablation is an honest null, and the positive control fires where theory says it should.** At permanent-impact exponents **0.5 and 0.7**, the symmetric pump remains unprofitable at every sampled point (three of 22 confidence intervals cross zero). An asymmetric schedule search then finds the profit nonlinear impact theory predicts: at exponent **0.5**, nine bars of slow accumulation followed by a one-bar block liquidation earns an impact-attributable **+21.8e-4** (CI [17.4, 26.2]) with no followers, its 1:9 mirror is deeply negative, exponent 0.7 shows the ordering but no profit, and all **45** linear-impact points are negative with exact time-reversal symmetry. The probe is falsifiable in both directions. The canonical follower ecology is never profitable, and the search did not cover push size, lambda, longer legs or short trips.
- **Bounded public seed bands are invertible; sealed seeds close that hole.** A causal AR adversary finds no undisclosed stressed-tier edge, but a scan of a public **2^16** seed band recovers **16/16** seeds from one observed bar in about one second. Unpredictability rests on seed entropy and secrecy, not transform hardness. The opt-in sealed-seed mechanism (`sealed_seed` / `sealed_eval_seeds`, a keyed PRF-style derivation into the held-out band under a commit-reveal salt) is the fix: the same scan recovers **0/16** sealed seeds, and revealing the salt replays **16/16**. The canonical split in [`EVALUATION.md`](EVALUATION.md) is for local comparison; a hosted evaluation seals.
- **Rank-eligibility is attainable, and pass^k is what binds.** An out-of-band oracle (the seed replayed one bar ahead, not an entrant, never scored) mixed with noise at controlled strength opens the gates on every tier: the eligibility region is non-empty, and at all ten attained boundaries the binding leg is the per-run PSR gate on every seed, which the pooled deflated-Sharpe and bootstrap legs clear long before. On Calm an every-bar sign follower never passes even with a perfect oracle, because flipping every bar costs about 14 bp/bar against 2.7 bp of omniscient sign value; a turnover-rationing variant passes from strength 0.90. `flat` fails 16/16 because a zero-dispersion series carries no evidence of edge (PSR 0.5), not because inactivity is penalized.
- **Single-seed ecology stories do not replicate.** Replicating the ecology probe over eight replicator seeds per schedule overturns its own earlier headline: the shock-driven replacement of the volatility targeter seen at seed 0 occurs on **1 of 8** seeds, while a `kelly_vol_target` lineage ends dominant on 7 of 8 shocked seeds. What the probe reliably produces is the outcome distribution, not one narrative.

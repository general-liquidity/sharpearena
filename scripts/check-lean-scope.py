#!/usr/bin/env python3
"""Require every Lean module under formal/SharpeArenaFormal/ to declare its scope.

A module's doc comment must carry a ``## Scope`` block with a ``Covers:`` line naming the
production symbols it models and an ``Assumes:`` line naming the assumptions the model
rests on, and at least one backticked repository path in the block must exist. A proof
whose scope is not written down reads as broader than it is; this check keeps every
module's coverage statement next to the theorems and fails CI when it is missing or
...
FORMAL = ROOT / "formal" / "SharpeArenaFormal"

...
The point of shipping the reference is that agents on this task are scored on regret rather than raw PnL. A market-making leaderboard ranked on PnL rewards whichever policy drew the friendliest flow; regret against a fixed analytical reference on the same episodes removes that luck axis when both arms see the same mid path. Identical seeds do not guarantee that path (\cref{sec:tasks}), so \texttt{mm\_regret} now checks it and refuses an unpaired comparison. A committed test runs every half-spread of this subsection through the checked function at the default parameters and reproduces the frozen regrets bit for bit, so every comparison here shares its mid path.

...
The \texttt{flat} rows are not scores. The flat book holds zero target weight on every bar, so its return series is identically zero on every seed of every tier, and its Sharpe ratio, a zero mean over a zero standard deviation, is undefined. The frozen artifact nonetheless reports a DSR of 0.0007 on each tier, with a zero-width interval: the SharpeBench 0.5.0 scorer that produced it replaces a zero standard deviation with a Sharpe ratio of zero, takes the skewness as zero and the kurtosis as three, and scores the result. The printed value is that convention's output for an undefined statistic, not evidence about the policy. The current SharpeArena code withholds the Sharpe ratio, PSR, DSR and the statistics built on them for a zero-variance track and records a named reason instead, so \texttt{flat} would now be reported as unavailable rather than scored. That refusal is SharpeArena's own; the pinned SharpeBench 0.21.0 scorer still returns a Sharpe ratio of zero for such a track.
% TODO(coordinator): reason name from paper/sa-kernel
% TODO(coordinator): mechanism from paper/sa-kernel. As of the 0.29.0 release tree this
% sentence is not yet true: leaderboard_ci.rs sharpe_ratio and the pinned
% sharpebench-stats 0.21.0 deflated_sharpe.rs:23 both return 0.0 on zero standard
% deviation and a finite DSR follows. Confirm the withholding lands before integration.

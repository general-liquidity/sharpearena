...
F1, F3, F7, F8 and the witness run on the canonical position-trading generator, whose tape the realism diagnostic of \cref{sec:f4} declines to certify on 23 of 24 seeded panels; F2 runs on the Avellaneda--Stoikov market-making model instead. Every number below is therefore a calibration of an instrument on tape the paper does not claim is market-realistic, and \cref{sec:limitations} carries that constraint forward into every claim built on it. No golden or parity test pins the F1--F3 outputs themselves, which come from Python rollouts through the native binding on one platform; what is tested is narrower. The native and Python suites pin the scenario generator at seven named specifications, and the canonical and clustered ones also reproduce on \texttt{wasm32} and in the shipped npm bundle (\texttt{scenario\_kernel\_matches\_native\_and\_every\_committed\_golden}, \texttt{exported\_generate\_scenario\_reproduces\_every\_committed\_golden}). \texttt{wasm\_kernel\_matches\_native\_engine} requires the WebAssembly JSON kernel, compiled for the host, to reproduce the native engine's returns and trace for one buy-and-hold fixture. The backtest and kernel goldens pin \texttt{run\_baseline}, \texttt{replay\_run}, \texttt{walk\_forward}, \texttt{stress\_suite} and \texttt{tag\_regime} outputs for named inputs natively, on \texttt{wasm32} and in the npm bundle (\texttt{backtest\_goldens\_reproduce\_natively}, \texttt{kernel\_goldens\_reproduce\_natively} and their \texttt{exported\_}\ldots\texttt{\_under\_wasm32} counterparts). The market-clearing engine's opt-in entry points are held to its linear path by native equality tests (\texttt{a\_unit\_exponent\_is\_byte\_identical\_to\_the\_linear\_path}, \texttt{absent\_uncertainty\_set\_is\_byte\_identical\_to\_the\_point\_estimate}). The Python evidence reductions are seed-deterministic on one platform but sit outside all of these. F1 and F3 report 2{,}000-resample seed bootstrap intervals, F2 reports t-based 95\% intervals over committed per-episode vectors, and F7 and F8 are count-based, reporting episode counts and seed-level winner distributions rather than interval estimates. Every entry point, seed set and quantity is named where it is reported, and the commands are in \cref{app:commands}.
...
Calm & \texttt{kelly\_vol\_target} & 1.00$^{\dagger}$ & $[0.98, 1.00]$ & 0.75 & 3.37 \\
 ...
 & \texttt{flat} & 0.00$^{\ddagger}$ & $[0.00, 0.00]$ & 0.00 & 0.00 \\
...
The \texttt{flat} rows are not scores. The flat book holds zero target weight on every bar, so its return series is identically zero on every seed of every tier, and its Sharpe ratio, a zero mean over a zero standard deviation, is undefined. The frozen artifact nonetheless reports a DSR of 0.0007 on each tier, with a zero-width interval: the SharpeBench 0.5.0 scorer that produced it replaces a zero standard deviation with a Sharpe ratio of zero, takes the skewness as zero and the kurtosis as three, and scores the result. The printed value is that convention's output for an undefined statistic, not evidence about the policy. A repair made after release 0.29.0 withholds these values at SharpeArena's own boundary, which it was written to do while the pinned scorer still substituted a Sharpe ratio of zero. The pinned scorer is now SharpeBench 0.28.0, which, like 0.27.0, refuses such a track itself, with the same reason and in the same fields, so the refusal no longer rests on SharpeArena's boundary alone: on every track that boundary refuses, the kernel has already recorded the refusal, and SharpeArena's check is a second line that changes no reported value. A track whose observations all equal the first, zero or not, is treated as having no Sharpe ratio; the test is exact equality rather than a volatility threshold, so a dispersed track of any volatility scores exactly as before. \texttt{score\_run} reports such a track with the typed unavailability \texttt{deflation\_error}, whose reason reads ``returns must not be constant: a constant series has no Sharpe ratio'', the reason the corresponding SharpeBench repair uses and the pinned kernel now writes itself. Its deflated Sharpe, PSR and deflation bar print the floor value 0 rather than an estimate, its bootstrap interval and rolling Sharpe summary are absent, and it fails the per-run reliability gate and rank eligibility. The seed-paired bootstrap interval and the paired-difference test refuse a constant pooled band with the same reason, and a track that is not constant but whose computed standard deviation underflows to zero is refused as ``Sharpe ratio is not finite''. The repair also closes a worse case of the same shape: a constant nonzero track has a tiny nonzero computed standard deviation, so before it a track of 0.001 on every bar scored a deflated Sharpe and PSR of 1.0 and was rank-eligible. Under the repaired code the \texttt{flat} rows report the reason in place of a deflated Sharpe, a $\passk$ rate and an interval. SharpeBench 0.27.0 carries a second repair of the same shape, which stops a track with no Sharpe ratio voting on other agents' deflation bar where that bar is measured from the field. No panel in this paper is affected, and no value in \cref{tab:f1} or in any other table here moves with it. This board's bar is not measured from a field: \texttt{run\_baselines} scores each of the six policies as its own one-agent submission through \texttt{score\_run}, and a one-agent submission deflates against the configured cross-trial dispersion prior in every SharpeBench release this paper has pinned. Every producer here that reaches the kernel at all reaches it that way; none calls the field-ranking entry point where a measured bar is possible. The \texttt{flat} track therefore never voted on another policy's bar. That holds by the producer path rather than by readback, because the committed artifacts do not serialize which dispersion source the scorer took.
...
\caption{Deflated Sharpe per baseline and tier. Error bars are seed-paired percentile bootstrap 95\% intervals ($B = 2{,}000$). Tiers differ by colour and by hatching (Calm plain, Hard diagonal, Extreme dotted). Calm saturates the long baselines; Hard and Extreme collapse them toward zero. The \texttt{flat} bars are the frozen artifact's value for an undefined statistic (\cref{sec:f1}).}
\label{fig:f1}
\end{figure}
...
\caption{Witness sweep on the gap-band witness subset under the primary noise path, one row per policy. Left panels: pooled deflated Sharpe against the nominal signal strength $s$; the band around each curve is a seed-paired percentile bootstrap 95\% interval (2{,}000 resamples), filled markers are rank-eligible sampled points and hollow markers are not, and the dotted line is the DSR bar of 0.95. Right panels: the $\passk$ rate over the 16 seeds, with a dotted line where all 16 pass. In every panel a dashed vertical line marks the primary-path bisection crossing, drawn only after monotonicity is verified, and a shaded column spans the minimum to maximum crossing over five independent noise paths; in the right panels a horizontal bar repeats that range with its marker at the mean. Tiers differ by marker (Calm circle, Hard square, Extreme triangle) and by column hatching (Calm plain, Hard diagonal, Extreme dotted).}
\label{fig:witness}
\end{figure}
...
The point of shipping the reference is that agents on this task are scored on regret rather than raw PnL. A market-making leaderboard ranked on PnL rewards whichever policy drew the friendliest flow; regret against a fixed analytical reference on the same episodes removes that luck axis when both arms draw the same random stream. Identical seeds do not guarantee that stream (\cref{sec:tasks}), so \texttt{mm\_regret} now checks it and refuses an unpaired comparison. A committed test runs every half-spread of this subsection through the checked function at the default parameters and reproduces the frozen regrets bit for bit, so every comparison here shares its stream.
...
\caption{Failure-mode counts per tier, each summed over the 8 policies and 16 seeds (128 episodes per tier, 384 in total), for every disposition in the rollup schema; the bankruptcy, cascade-wipe, invalid-evidence and execution-failure bars are zero. Tiers differ by colour and by hatching (Calm plain, Hard diagonal, Extreme dotted).}
\label{fig:f7}
\end{figure}
...
\end{subfigure}
\caption{Seed-0 population-share trajectories over 12 replicator generations, the illustrative single run behind \cref{tab:f8}'s distribution; seed 0 only. Both schedules start from the same eight founders (\texttt{flat}, \texttt{equal\_weight\_long}, \texttt{momentum}, \texttt{min\_variance}, \texttt{max\_sharpe}, \texttt{kelly\_vol\_target}, \texttt{disposition\_effect} and \texttt{overconfident}). The innovation operator adds three bred variants to each: \texttt{kelly\_vol\_target} variants labelled \textasciitilde\texttt{lookback4}, \textasciitilde\texttt{target\_vol8} and \textasciitilde\texttt{kelly\_fraction12} under control, and \texttt{equal\_weight\_long} variants labelled \textasciitilde\texttt{v4}, \textasciitilde\texttt{v8} and \textasciitilde\texttt{v12} under shocks. Every band is labelled directly on the plot with a leader line and carries its own hatch; there is no legend. At this seed the regime rotation replaces the calm-regime winner with the equal-weight long book; across the eight-seed replication that replacement occurs once.}
\label{fig:f8}
\end{figure}

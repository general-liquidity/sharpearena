...
\end{verbatim}
The command replays each seed one bar ahead to recover the true next-bar return (asserting that the canonical tape is policy-invariant), mixes it with seeded noise at 13 coarse strengths, scores \texttt{sign\_follow} and \texttt{deadband\_hold} through \texttt{score\_run} on every seed and on the pooled series, and bisects each tier's boundary to a resolution of $0.005$ in $s$ on the held-out band and the F1 table band. The JSON records every strength's per-seed PSR, pooled DSR, bootstrap $p$-value, eligibility flag and the bracketed boundaries behind \cref{tab:witness}.

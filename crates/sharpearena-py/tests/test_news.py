...
Dict-obs stub env; an optional live test runs against ``SharpeArenaEnv`` when the binding
imports.

    python -m pytest crates/sharpearena-py/tests/test_news.py -q
"""
...
from sharpearena.news import SyntheticNewsObservation, news_series


class _StubEnv(gym.Env):
    """Minimal Dict-obs env mirroring SharpeArenaEnv's price-only observation."""
...
def test_live_wrapped_env():
    ...
    base = SharpeArenaEnv(n_symbols=3, n_days=40, seed=4)

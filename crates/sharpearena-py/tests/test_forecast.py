"""Tests for the FinPILOT calibrated forecast-skill axis (``sharpearena.forecast``).

The calibration math (``calibrated_forecast`` / ``oos_r2``) is numpy-only and always
runs. The env-wrapper and curve tests need the native binding via ``SharpeArenaEnv``
...
from sharpearena.forecast import (
    calibrated_forecast,
    oos_r2,
...
def _make_env(seed: int):
    ...
    return SharpeArenaEnv(n_symbols=3, n_days=80, seed=seed)


def _binding_ok() -> bool:
    try:
        from sharpearena import SharpeArenaEnv  # noqa: F401
        from sharpearena.sharpearena_py import score_run  # noqa: F401
    except Exception:
        return False
    return True

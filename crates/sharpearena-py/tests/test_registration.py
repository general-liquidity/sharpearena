"""Tests for the gymnasium registration layer (``sharpearena.registration``).
...
from sharpearena import registration
from sharpearena.registration import register_envs, env_ids, EVAL_SEED_BASE
...
def _env_accepts_distribution_mode() -> bool:
    ...
    try:
        ...
        return False
    return "distribution_mode" in inspect.signature(SharpeArenaEnv.__init__).parameters


_needs_dist_mode = pytest.mark.skipif(
    not _env_accepts_distribution_mode(),
    reason="SharpeArenaEnv has no distribution_mode kwarg yet (parallel CORE-ENV stream)",
)
...
def test_env_ids_matrix():
    ...
    assert ids == [
        "SharpeArena/Calm-v1",
        "SharpeArena/Hard-v1",
        "SharpeArena/Extreme-v1",
        "SharpeArena/Calm-Eval-v1",
        "SharpeArena/Hard-Eval-v1",
        "SharpeArena/Extreme-Eval-v1",
    ]
...
def test_tier_kwargs_match_id():
    ...
    for tier in registration.TIERS:
        spec = gymnasium.registry[f"SharpeArena/{tier.capitalize()}-v1"]
...
def test_eval_variants_select_disjoint_band():
    ...
    for tier in registration.TIERS:
        spec = gymnasium.registry[f"SharpeArena/{tier.capitalize()}-Eval-v1"]
...
def test_make_vec_routes_to_vector_env():
    ...
    env = gymnasium.make_vec("SharpeArena/Hard-v1", num_envs=8)
    try:
        assert isinstance(env.unwrapped, SharpeArenaVectorEnv)

"""Conformance + determinism tests for the SharpeArena Python distribution.
...
import sharpearena
from sharpearena import SharpeArenaEnv, TradingEnv


def _equal_weight_action(env: SharpeArenaEnv) -> np.ndarray:
    n = env.action_space.shape[0]
    return np.full((n,), 1.0 / n, dtype=np.float32)


def test_reexports_present():
    assert hasattr(sharpearena, "TradingEnv")
    assert hasattr(sharpearena, "SharpeArenaEnv")
...
def test_reset_returns_observation():
    env = SharpeArenaEnv(n_symbols=4, n_days=60, seed=3)
    obs, info = env.reset()
...
def test_full_episode_finite_rewards_and_terminates():
    env = SharpeArenaEnv(n_symbols=4, n_days=60, seed=5)
    env.reset()
...
def test_determinism_same_seed_identical_rewards():
    def run(seed: int) -> list[float]:
        env = SharpeArenaEnv(n_symbols=4, n_days=80, seed=seed)
        env.reset()
        action = _equal_weight_action(env)
        out = []
        done = False
...
def test_reset_seed_selects_scenario():
    ...
    env = SharpeArenaEnv(n_symbols=3, n_days=60, seed=0)
    action = _equal_weight_action(env)
...
def test_hard_distribution_diverges_from_calm():
    def rollout(mode: str) -> list[float]:
        env = SharpeArenaEnv(n_symbols=4, n_days=60, seed=5, distribution_mode=mode)
        env.reset()
        action = _equal_weight_action(env)
        out = []
        done = False
...
def test_unknown_distribution_mode_rejected():
    with pytest.raises(Exception):
        SharpeArenaEnv(n_symbols=3, n_days=40, seed=1, distribution_mode="bogus")

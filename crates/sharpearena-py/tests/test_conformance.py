"""Conformance, causal-wrapper, and generalization tests for SharpeArena (Stream C).

Run from the crate dir after ``python -m maturin develop``::

    python -m pytest crates/sharpearena-py/tests/test_conformance.py -q
"""
...
from sharpearena.check_env import check_env, check_determinism_across_constructors
from sharpearena.wrappers import (
    TimeLimit,
...
from sharpearena.generalization import (
    train_test_seeds,
    evaluate_seeds,
    generalization_gap,
...
def _make(seed: int) -> SharpeArenaEnv:
    return SharpeArenaEnv(n_symbols=3, n_days=50, seed=seed)


def _make_mode(seed: int, mode: str) -> SharpeArenaEnv:
    return SharpeArenaEnv(n_symbols=3, n_days=50, seed=seed, distribution_mode=mode)

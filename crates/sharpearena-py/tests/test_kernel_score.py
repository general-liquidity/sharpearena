...
no-skill floor (the exact shape SharpeBench 0.19.0 serializes for a non-finite
observation) and asserts the reason surfaces where the consumer routes it: a
recorded reason string in row-shaped outputs, ``KernelScoreUnavailable`` from
...
def _floored(*keys: str) -> dict:
    """The 0.19.0 shape: floor values beside the named typed error(s)."""

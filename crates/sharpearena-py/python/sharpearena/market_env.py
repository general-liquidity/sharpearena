"""A PettingZoo **endogenous shared-book market** over SharpeArena (M2).
...
:class:`~sharpearena.pettingzoo_env.MultiAgentSharpeArenaEnv` — there every agent trades
...
split), implemented in pure deterministic Rust (``sharpearena::market``):
...
:mod:`sharpearena.pettingzoo_env`): ``import sharpearena`` and ``import
sharpearena.market_env`` both work without it; only *constructing*
...
class EndogenousMarketEnv(ParallelEnv):
    ...
    metadata = {
        "render_modes": [],
        "name": "sharpearena_endogenous_market_v0",
        ...
        *,
        ...
        if not _HAS_PETTINGZOO:
            raise RuntimeError(
                ...
                "EndogenousMarketEnv; the rest of the sharpearena package works without it."
            )
        if int(n_agents) < 1:
    ...
    def _build_spaces(self) -> None:
        ...
        # Mirrors sharpearena.gym.SharpeArenaEnv's decoded-observation Dict space.

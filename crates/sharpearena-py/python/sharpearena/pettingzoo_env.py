"""A PettingZoo multi-agent **competition** environment over SharpeArena.

:class:`MultiAgentSharpeArenaEnv` is a :class:`pettingzoo.ParallelEnv` that runs a
*batched tournament*: ``n_agents`` agents each trade their **own** copy of the
**same frozen scenario** (every per-agent :class:`~sharpearena.gym.SharpeArenaEnv` is
...
``import sharpearena`` and ``import sharpearena.pettingzoo_env`` both work without it; only
*constructing* :class:`MultiAgentSharpeArenaEnv` (or calling :func:`make_aec_env`) raises a
...
class MultiAgentSharpeArenaEnv(ParallelEnv):
    """A batched-competition :class:`pettingzoo.ParallelEnv` over SharpeArena.

    ``n_agents`` agents each own a private :class:`~sharpearena.gym.SharpeArenaEnv` seeded to
    ...
    metadata = {"render_modes": [], "name": "sharpearena_competition_v0", "is_parallelizable": True}
        ...
        *,
        ...
        if not _HAS_PETTINGZOO:
            raise RuntimeError(
                ...
                "MultiAgentSharpeArenaEnv; the rest of the sharpearena package works "
                "without it."
            )
        if int(n_agents) < 1:
        ...
        self._envs: dict[str, SharpeArenaEnv] = {}
...
def make_aec_env(*args: Any, **kwargs: Any):
    ...
    Accepts the same arguments as :class:`MultiAgentSharpeArenaEnv`. Raises a clear
    ``RuntimeError`` when ``pettingzoo`` is not installed.
    """
    if not _HAS_PETTINGZOO:
        raise RuntimeError(
            "pettingzoo is not installed. Install 'pettingzoo' to build the AEC view; "
            "the rest of the sharpearena package works without it."
        )
    return parallel_to_aec(MultiAgentSharpeArenaEnv(*args, **kwargs))

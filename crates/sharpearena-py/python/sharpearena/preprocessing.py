"""Canonical, declarative observation-preprocessing defaults for SharpeArena.
...
# The SharpeArena standard input pipeline. Freeze this; report deviations from it.
...
def make_preprocessed_env(
    ...
    """Build an :class:`SharpeArenaEnv` and apply the config's wrapper stack in order.

    Wrapper order (outermost last) — each layer wraps the one before it:

    1. ``SharpeArenaEnv(**env_kwargs)`` — the base point-in-time env.
    ...
    env: gym.Env = SharpeArenaEnv(**env_kwargs)
...
def describe_preprocessing(config: Optional[PreprocessingConfig] = None) -> str:
    ...
    lines = [
        "SharpeArena preprocessing config",
    ...
    lines.append(
        "  -> CANONICAL SharpeArena pipeline (no disclosure needed)."
        if canonical

"""Export a recorded SharpeArena rollout trace to a `Minari <https://minari.farama.org>`_
offline-RL dataset.

Minari (Farama) is the offline-RL trajectory dataset standard. This module converts the
versioned JSONL rollout trace produced by :mod:`sharpearena.trace`
(:class:`~sharpearena.trace.RolloutTraceWriter`) into a :class:`minari.MinariDataset`
...
reset observation plus one observation after each of the ``T`` steps. The SharpeArena trace
...
**Leak-safety** carries over from :mod:`sharpearena.trace`: a raw dataset/env handle in any
...
def _require_minari() -> None:
    if not _HAS_MINARI:
        raise RuntimeError(
            ...
            "(pip install 'sharpearena[minari]') to export rollout traces to an offline-RL "
            "dataset; the rest of the sharpearena package works without it."
        )
...
def to_minari(
    ...
    **kwargs: Any,
    ...
    """Convert a recorded SharpeArena rollout ``trace`` into a :class:`minari.MinariDataset`.

    Parameters
    ----------
    trace:
        A JSONL trace path, an in-memory ``(records, meta)`` tuple (e.g. from
        :func:`sharpearena.trace.load_trace`), or a bare list of step records.
    dataset_id:
    ...
    observation_space, action_space:
        ...
        dataset WITHOUT a live env. Use ``SharpeArenaEnv(...).observation_space`` /
        ``.action_space``.
    terminated:
    ...
    env:
        ...
        such as ``"SharpeArena/<Tier>-v1"`` once the gymnasium ids are registered to make
...
def to_minari_train_test(
    ...
    **kwargs: Any,
    ...
    :func:`minari.combine_datasets`, but a random split would tear SharpeArena's leak-safe
    ...
    :data:`~sharpearena.dataset.EVAL_SEED_BASE`, test at/above it) and stamps each dataset's

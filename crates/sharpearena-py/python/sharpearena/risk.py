"""Risk-termination and circuit-breaker wrappers over :class:`~sharpearena.gym.SharpeArenaEnv`.
...
on ``reset``, the :mod:`sharpearena.indicators` pattern). None can leak a future bar into a
present decision.
"""
...
class CrossSectionalDeleverage(gym.Wrapper):
    ...
    :mod:`sharpearena.indicators`) from a causal per-episode buffer of the ``closes`` the base

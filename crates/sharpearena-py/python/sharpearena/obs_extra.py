"""Extra causal observation-feature wrappers for :class:`SharpeArenaEnv`.

Cheap, leak-free augmentations that SharpeArena's near-raw obs lacks. Each reads the
...
:mod:`sharpearena.indicators` — so the value emitted at bar ``t`` depends only on closes

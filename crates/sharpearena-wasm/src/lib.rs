//! WASM façade over [`sharpearena`] — the trading-agent environment (leak-free
//! point-in-time simulator + reference baselines + the recompute-to-verify path) —
//! so TypeScript/Bun (the published `@general-liquidity/sharpearena` npm package)
...
/// returning the [`sharpearena::Run`] JSON (per-period returns + decision trace +
...
/// to regenerate its [`sharpearena::Run`] — the tamper-evidence path. The `Run` is
...
mod tests {
    ...
    fn replay_recomputes_the_same_run() {
        ...
        use sharpearena::{run_backtest_capture, Dataset as Ds, Window as W};
        ...
        let replayed: sharpearena::Run = serde_json::from_str(&out).unwrap();
        assert_eq!(
    ...
    fn wasm_kernel_matches_native_engine() {
        use sharpearena::{run_backtest, BuyAndHold, Dataset as Ds, Window as W};

"""Equivalence gate: the batched OpenOutcry path is the same env as the scalar path.
...
wrapper. These tests make that statement executable for OpenOutcry:

- ``test_batched_equals_scalar`` drives a ``B``-lane ``OpenOutcryVectorEnv`` and ``B`` separate
  ``OpenOutcryEnv`` over identical per-lane actions to episode end and asserts the
...
Seed note: ``OpenOutcryEnv`` splits each user seed into decorrelated (scenario, execution)
streams via ``SeedSequence``; ``OpenOutcryVectorEnv`` takes the lane (scenario) seeds
...
def test_batched_equals_scalar_under_execution_noise(seed):
    n_symbols, n_days = 3, 40
    scalar = OpenOutcryEnv(seed=seed, n_symbols=n_symbols, n_days=n_days)
...
def test_step_async_step_wait_equals_step():
    n_symbols = 3
    a = OpenOutcryVectorEnv(seeds=[2, 3, 4], n_symbols=n_symbols, n_days=30)
    b = OpenOutcryVectorEnv(seeds=[2, 3, 4], n_symbols=n_symbols, n_days=30)
    a.reset()
    b.reset()
    for _ in range(20):
...
def test_step_wait_without_pending_raises():
    env = OpenOutcryVectorEnv(seeds=[1], n_symbols=2, n_days=20)
    env.reset()
